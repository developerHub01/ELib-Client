    ...
    const loadBookData = async () => {
      axios
        ...
        .then((res) => {
          ...
          console.log(_id, email, returnDate, id, borrowedDate);
        })
        .catch((error) => {
          console.log(error.message);
          setBookData((prev) => false);
        });
    };
    if (id) loadBookData();
  }, [isLoading, id]);
    ...
    <>
      {bookData._id ? (
        <div className="w-full h-full flex flex-col p-5 bg-gray-950 rounded-xl gap-3 shadow-2xl group">
          ...
          <div className="w-full flex flex-col gap-2 justify-center items-start text-white flex-auto">
            <Link
              to={`/read/${id}`}
